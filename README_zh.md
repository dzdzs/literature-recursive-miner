...
./run_lrm_zh.sh /mnt/userdata/huashengjia/lrm_run_001
```
...
`run_lrm_zh.sh` 会从**当前执行目录**加载 `.env`，即 `$(pwd)/.env`。

示例：如果你在其他目录运行脚本，脚本会读取那个目录下的 `.env`：

```bash
cd /some/dir/with/env
/mnt/userdata/huashengjia/literature-recursive-miner/run_lrm_zh.sh /tmp/lrm_out
```
